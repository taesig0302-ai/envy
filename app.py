...
# =========================
# 7) DataLab Category → Top20 & Trend  (12개 카테고리 버전)
# =========================
def section_category_keyword_lab():
    st.markdown('<div class="card"><div class="card-title">카테고리 → 키워드 Top20 & 트렌드</div>',
                unsafe_allow_html=True)
    ...
    SEED_MAP_12 = {
        ...
        "출산/육아":  ["기저귀","분유","젖병","아기띠","유모차"],
        "식품":      ["간편식","커피","간식","라면","캔음료"],
        "생활/건강":  ["칫솔","치약","샴푸","세제","물티슈"],
        "가전/디지털":["블루투스이어폰","스피커","모니터","노트북","로봇청소기"],
        "스포츠/레저":["러닝화","요가복","캠핑의자","텐트","자전거"],
        "도서/문구":  ["볼펜","노트","다이어리","수첩","풀"],
        "자동차/공구": ["블랙박스","차량용거치대","세차용품","엔진오일","타이어압측정기"],
        "여행/문화":  ["캐리어","여행가방","목베개","보조배터리","수영복"],
        "반려동물/취미":["강아지사료","고양이모래","장난감","스크래처","간식"],
    }

    cA, cB, cC = st.columns([1,1,1])
    with cA:
        cat = st.selectbox("카테고리", list(SEED_MAP_12.keys()), key="cat12_sel")
    with cB:
        time_unit = st.selectbox("단위", ["week","month"], index=0, key="cat12_unit")
    with cC:
        months = st.slider("조회기간(개월)", 1, 12, 3, key="cat12_months")

    # Top20 표
    seeds = SEED_MAP_12.get(cat, [])
    ...
    # 트렌드 라인차트
    topk = st.slider("라인차트 키워드 수", 3, 10, 5, key="cat12_topk",
                     help="상위 N개 키워드만 트렌드를 그립니다.")
