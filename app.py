...
def _inject_css():
    """메인 뷰만 색상 오버라이드(사이드바 제외). 다크/라이트 대비 강화 + 특정 위젯 타겟팅."""
    ...
    # 다크모드에서 '검정으로 강제할 대상' 목록 (라벨 텍스트와 정확히 일치)
    # - 레이더: 디바이스, 키워드 소스
    # - 카테고리 섹션: 카테고리, 단위
    force_black_labels = ["디바이스", "키워드 소스", "카테고리", "단위"]

    # aria-label 선택자 문자열 생성
    aria_targets = ", ".join([f"[aria-label='{t}']" for t in force_black_labels])
    ...
    <style>
      ...
      [data-testid="stAppViewContainer"] .stMarkdown, [data-testid="stAppViewContainer"] .stMarkdown * {{
        color:{fg} !important;
      }}
      ...
      /* === 핵심: 다크모드일 때 특정 라벨(카테고리/단위/디바이스/키워드 소스)만 검정 폰트 강제 === */
      {("""
        [data-testid='stAppViewContainer'] {aria_targets} *,
        [data-testid='stAppViewContainer'] label:has(+ {aria_targets}),
        [data-testid='stAppViewContainer'] {aria_targets} label {{
      ...
      """.replace("{aria_targets}", aria_targets) if theme == "dark" else "")}
    </style>
