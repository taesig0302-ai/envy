...
# -*- coding: utf-8 -*-
# ENVY v19 — Streamlit full app (no-blank safe defaults)
...
st.set_page_config(page_title="ENVY — v19", page_icon="🦊", layout="wide")
...
.envy-top { display:flex;align-items:center;gap:12px;margin:6px 0 14px 0; }
.envy-logo { width:36px;height:36px;border-radius:10px;background:#FF6B00;display:flex;align-items:center;justify-content:center;font-weight:800;color:white; }
.envy-title { font-size:26px; font-weight:800; }
.envy-sub { opacity:.65 }
.stMetricValue, .stMetricDelta { color: inherit !important; }
.anchor { scroll-margin-top: 70px; }
</style>
""", unsafe_allow_html=True)
    st.markdown(r"""
<div class="envy-top">
  <div class="envy-logo">E</div>
  <div class="envy-title">ENVY</div>
  <div class="envy-sub">소싱 · 키워드 · 가격 — v19</div>
</div>
""", unsafe_allow_html=True)
...
fx_amt = st.sidebar.number_input("상품 원가", min_value=0.0, value=1.0, step=1.0)
fx_cur = st.sidebar.selectbox("통화", ["USD ($)","EUR (€)","JPY (¥)","CNY (¥)"], index=0)
REF = {"USD ($)":1391.7,"EUR (€)":1510.0,"JPY (¥)":9.2,"CNY (¥)":191.3}
st.sidebar.info(f"환율: 1 {fx_cur.split()[0]} ≈ ₩{REF[fx_cur]:,.2f}")
st.sidebar.success(f"원화 환산: ₩{(fx_amt*REF[fx_cur]):,.0f}")

# ---------------------- (1) DataLab API + Graph ----------------------
st.markdown("## 📊 <span class='anchor' id='datalab'></span>네이버 데이터랩 (API + 그래프)", unsafe_allow_html=True)
with st.expander("API 키 설정"):
    ...
    st.caption("※ 키가 없으면 데모 모드로 동작합니다.")
...
try:
    if (cid and csec):
        df_ts = datalab_search_trend(cid, csec, SEED[cat], start_date.isoformat(), end_date.isoformat())
        tops = []
...
with st.expander("📈 트렌드 그래프 / CSV"):
    if cid and csec and not df_top.empty:
        pick = df_top["keyword"].head(5).tolist()
        frames = []
        for kw in pick:
            dfp = datalab_search_trend(cid, csec, [kw], start_date.isoformat(), end_date.isoformat())
            dfp["keyword"] = kw
            frames.append(dfp)
        if frames:
            ...
            st.download_button("CSV 다운로드", data=df_plot.to_csv(index=False).encode("utf-8-sig"), file_name=f"datalab_{cat}_{days}d.csv", mime="text/csv")
    else:
        st.info("API 키 입력 시 실측 그래프가 표시됩니다.")

st.markdown("---")

# ---------------------- (2) Itemscout ----------------------
st.markdown("## 🔎 아이템스카우트 — API/CSV/HTML")
col_is1, col_is2 = st.columns([1,1])
with col_is1:
    use_is_api = st.toggle("API 사용", value=False, help="무료 플랜은 API 제한일 수 있음")
    is_key = st.text_input("Itemscout API Key", type="password", disabled=not use_is_api)
    is_kw = st.text_input("조회 키워드", value="슬랙스", disabled=not use_is_api)
    if st.button("API 호출", disabled=not use_is_api):
        try:
            raise RuntimeError("무료 플랜/API 스펙 제한 — CSV/HTML 모드 사용 권장")
        except Exception as e:
            st.error(f"API 호출 실패: {e}")

with col_is2:
    st.caption("CSV 업로드 (Itemscout 내보내기)")
    csvfile = st.file_uploader("CSV 업로드", type=["csv"])
    if csvfile:
        try:
...
st.caption("HTML 붙여넣기(공개 목록 페이지 소스) → 키워드 추출")
html_txt = st.text_area("HTML 소스", height=120)
...
# ---------------------- (3) 11st Reader ----------------------
st.markdown("## 🛍️ 11번가 리더 모드 (모바일 요약/표)")
url_11 = st.text_input("URL", value="https://m.11st.co.kr/browsing/AmazonBest")
if st.button("불러오기"):
    try:
...
# ---------------------- (5) Title Generator + (6) Banwords ----------------------
st.markdown("## ✍️ 상품명 생성기 + 🚫 금칙어")
colg1, colg2 = st.columns([1.2,1])
with colg1:
    brand = st.text_input("브랜드", value="")
    base = st.text_input("기본 문장", value="")
    kw_raw = st.text_input("키워드(,)", value="슬랙스, 와이드, 기모")
...
with colg2:
    ...
    ban_df = st.data_editor(st.session_state["ban_df"], num_rows="dynamic", use_container_width=True)
...
def gen_titles(brand, base, kws, rules, limit_chars, limit_bytes, n=5):
    ...
    for i in range(n):
        ...
        while (ch > limit_chars or bt > limit_bytes) and kk:
            ...
            title = apply_rules(title, rules)
            ch, bt = count_kor_bytes(title)
        out.append({"title": title, "chars": ch, "bytes": bt})
...
if st.button("제목 5개 생성"):
    ...
    st.download_button("CSV 다운로드", df_titles.to_csv(index=False).encode("utf-8-sig"), "titles.csv", "text/csv")

st.caption("© ENVY v19 — DataLab(API) · Itemscout(CSV/HTML) · 11st Reader · Radar · Titles/Rules")
