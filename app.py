# -*- coding: utf-8 -*-
# ENVY — Season 1 (Dual Proxy Edition, Final Patch w/ smaller logo + JS fix)
# - ItemScout/SellerLife: 보류 카드(임베드 대신 안내 + 새 탭 버튼) 유지
# - Rakuten: cache(15분) + 백오프 재시도, 랭크 2단계 축소, 가로 스크롤 제거, CSV 다운로드
# - DataLab: 외부 스크롤 억제 + 탭 제목 수신 유지 (Template 사용으로 JS 중괄호 충돌 해결)
...
def _get_view_bin():
    try:
        raw = st.query_params.get("vwbin", "3")  # Streamlit 최신
    except Exception:
        raw = (st.experimental_get_query_params().get("vwbin", ["3"])[0])  # 구버전 호환
    try:
    ...
    <script>
      (function(){
        ...
        window.addEventListener("message", function(e){
          const d = e.data || {};
          try { if (d.__envy && d.kind === "title" && d.title) titleEl.textContent = d.title; } catch(_) {}
        }, false);
      })();
    </script>
    """).substitute(key=key, url=url, h=h)
...
def _rk_guess_genre_by_keyword(jp_keyword: str, hits: int = 30) -> Optional[str]:
