...
def section_11st():
    ...
    ss = st.session_state
    ss.setdefault("__11st_token", str(int(time.time())))
    colr, colx = st.columns([1,1])
    with colr:
        if st.button("🔄 새로고침 (11번가)", key="btn_refresh_11st"):
            ss["__11st_token"] = str(int(time.time()))
    with colx:
        no_sandbox = st.toggle("호환 모드(샌드박스 해제)", value=False, help="프레임버스터 대응")
    ...
    iframe_attrs = (
        'referrerpolicy="no-referrer" '
        'style="width:100%;height:100%;border:0;border-radius:10px;background:transparent;"'
    )
    if not no_sandbox:
        iframe_attrs += ' sandbox="allow-scripts allow-forms allow-popups allow-pointer-lock allow-same-origin allow-top-navigation-by-user-activation"'

    html = f"""
    <style>.embed-11st-wrap{{height:940px;overflow:hidden;border-radius:10px;}}</style>
    <div class="embed-11st-wrap">
      <iframe src="{want}" title="11st" {iframe_attrs}></iframe>
    </div>"""
    st.components.v1.html(html, height=960, scrolling=False)
    st.link_button("새 탭으로 열기(프록시 확인)", want, use_container_width=True)
