...
# 패치 요약
# - [섹션10-1 교체] 11번가 임베드: 상단 얇은 헤더(새로고침/프록시표시/새탭 버튼), iFrame 토큰-리로드
# - [섹션10-2 추가] 11번가 상품상세 미리보기: 내장 미리보기 + 새탭(프록시) + 원본 열기
# - 라이트/다크 가독성 패치 유지, 레이더/카테고리/생성기 동일
# - 프록시 비어있으면 안내만 표시

import base64, time, re, math, json, io, datetime as dt
from pathlib import Path
from urllib.parse import quote as _q, quote
import pandas as pd
...
ELEVENST_PROXY = "https://worker-11stjs.taesig0302.workers.dev"  # << 너 워커 주소로 교체
...
def _inject_css():
    ...
    if theme == "dark":
        ...
        [data-testid="stAppViewContainer"] textarea::placeholder{ color:#6b7280 !important; opacity:1 !important; }
        """
        pill_rules = """
        ...
        .pill.pill-blue, .pill.pill-blue *{ color:#fff !important; -webkit-text-fill-color:#fff !important; }
        """
        force_black_rules = ""
    ...
    {force_black_rules}
    /* 11번가 헤더 라인 정리 */
    .thin-row{{ display:flex; gap:.5rem; align-items:center; margin-bottom:.4rem; flex-wrap:wrap; }}
    .thin-row .muted{{ font-size:.85rem; opacity:.8 }}
    .thin-row .spacer{{ flex:1 }}
    </style>
...
def section_korea_ui():
    ...
    with c1: st.slider("분석기간(개월, 표시용)", 1, 6, 3)
    with c2: st.selectbox("디바이스", ["all","pc","mo"], index=0)
    with c3: st.selectbox("키워드 소스", ["직접 입력"], index=0)
    ...
    if st.button("레이더 업데이트", use_container_width=False):
        ...
        if table_mode.startswith("A"):
            st.dataframe(df2, use_container_width=True, height=430)
            st.download_button("CSV 다운로드", df2.to_csv(index=False).encode("utf-8-sig"), file_name="korea_keyword_A.csv", mime="text/csv")
            st.markdown("</div>", unsafe_allow_html=True); return

        df2["검색합계"] = (pd.to_numeric(df2["PC월간검색수"], errors="coerce").fillna(0) + pd.to_numeric(df2["Mobile월간검색수"], errors="coerce").fillna(0))
        ...
        if table_mode.startswith("B"):
            ...
            st.download_button("CSV 다운로드", out.to_csv(index=False).encode("utf-8-sig"), file_name="korea_keyword_B.csv", mime="text/csv")
            st.markdown("</div>", unsafe_allow_html=True); return
        ...
        if add_product:
            with st.spinner("네이버쇼핑 상품수 수집 중…(키워드 수에 따라 수 분 소요)"):
                for k in df2["키워드"]:
                    cnt = _count_product_from_shopping(k); product_counts.append(cnt if cnt is not None else math.nan)
        else:
        ...
        st.dataframe(out, use_container_width=True, height=430)
        st.download_button("CSV 다운로드", out.to_csv(index=False).encode("utf-8-sig"), file_name="korea_keyword_C.csv", mime="text/csv")
    st.markdown("</div>", unsafe_allow_html=True)
...
def _datalab_trend(groups: list, start_date: str, end_date: str, time_unit: str = "week",
    ...
    try:
        ...
        st.error(f"DataLab 오류: {e}"); return pd.DataFrame()
...
def _stopwords_manager_ui(compact: bool = False):
    ...
    if not compact:
        ...
        tab_global, tab_cat, tab_white, tab_replace, tab_io = st.tabs(["전역 금칙어", "카테고리 금칙어", "화이트리스트", "치환 규칙", "가져오기/내려받기"])
        with tab_global:
        ...
        with tab_io:
            payload = {
                "global": ss["STOP_GLOBAL"], "by_cat": ss["STOP_BY_CAT"],
                "whitelist": ss["STOP_WHITELIST"], "replace": ss["STOP_REPLACE"],
...
def _suggest_keywords_by_searchad_and_datalab(seed_kw:str, months:int=3, top_rel:int=15, strict:bool=True) -> pd.DataFrame:
    ...
    for i in range(0,len(df),5):
        ...
        if ts.empty: [dl_means.setdefault(k,0.0) for k in chunk]
        else:
            for k in chunk:
...
def section_title_generator():
    ...
    with cA:
        st.text_input("브랜드", placeholder="예: 무지 / Apple")
...
# =========================
# 10-1) 11번가 — 아마존 베스트(임베드 · 교체)
# =========================
def section_11st_embed():
    ...
    # 헤더 라인(얇게): 새로고침 / 프록시 주소 / 새탭 버튼들
    st.markdown('<div class="thin-row">', unsafe_allow_html=True)
    col1, col2, col3 = st.columns([1.2, 3, 2.2])
    with col1:
        if st.button("🔄 새로고침", key="btn_refresh_11st"):
            ss["__11st_token"] = str(int(time.time()))
    with col2:
        if base_proxy:
            st.markdown(f'<span class="muted">PROXY: <a href="{base_proxy}" target="_blank">{base_proxy}</a></span>', unsafe_allow_html=True)
        else:
            st.markdown('<span class="muted">PROXY 없음 — 워커 주소가 비었습니다.</span>', unsafe_allow_html=True)
    with col3:
        proxied = (raw_url if not base_proxy else f"{base_proxy}/?url={_q(raw_url, safe=':/?&=%')}")
        st.link_button("새 탭(프록시)", proxied, use_container_width=True)
    st.markdown('</div>', unsafe_allow_html=True)

    if not base_proxy:
        st.info("프록시가 설정되지 않아 iFrame 임베드가 차단될 수 있어요. 상단 버튼으로 새 탭(프록시)로 열어 보세요.")
        st.markdown("</div>", unsafe_allow_html=True); return
...
def section_11st_detail():
    ...
    c1, c2, c3 = st.columns([1,1,1])
    with c1:
        st.link_button("원본 열기(직접)", target or "https://m.11st.co.kr/", use_container_width=True, disabled=not target)
    with c2:
        st.link_button("새 탭(프록시)", proxied or "", use_container_width=True, disabled=not proxied)
    with c3:
        if st.button("상세 미리보기(내장)", use_container_width=True, disabled=not proxied):
            if proxied:
                html = f"""
                <style>
                  .embed-11st-detail {{ height: 900px; overflow: hidden; border-radius: 10px; border:1px solid rgba(0,0,0,.06); }}
                ...
                <div class="embed-11st-detail">
                  <iframe src="{proxied}" title="11st-detail"></iframe>
                </div>
                """
                st.components.v1.html(html, height=920, scrolling=False)

    with st.expander("도움말 / 파싱 규칙", expanded=False):
        st.write("- `https://www.11st.co.kr/products/123` / `https://m.11st.co.kr/products/123` / `...productId=123` 모두 인식합니다.")
        st.write("- 숫자만 입력해도 됩니다. (예: `1234567890`)")
        st.write("- 프록시가 없으면 임베드는 차단될 수 있어요. 그 경우 ‘새 탭(프록시)’로 여세요.")
    st.markdown("</div>", unsafe_allow_html=True)
...
# row2: 11번가(임베드) | 아이템스카우트 | 셀러라이프
c1, c2, c3 = st.columns([3, 3, 3], gap="medium")
with c1: section_11st_embed()         # [교체]
with c2: section_itemscout_placeholder()
with c3: section_sellerlife_placeholder()
st.markdown('<div class="row-gap"></div>', unsafe_allow_html=True)

# row3: 11번가 상세 보기(추가 섹션)
st.markdown("### 11번가 상품 상세 (직접 입력 → 프록시 미리보기/새 탭)")
section_11st_detail()
